...
class RotationAlgorithm:
    def __init__(self):
        ...
        pygame.display.set_caption("2D Point Rotation")
        self.clock = pygame.time.Clock()
        self.font = pygame.font.Font(None, 20)
        self.scaling_factor = 20  # Increase the scaling factor for more spacing

    def draw_axes(self, initial_point, rotated_point, angle):
        ...
        self.draw_point(initial_point, (0, 255, 0), 'Initial Point')

        # Draw the rotated point
        self.draw_point(rotated_point, (255, 0, 0), 'Rotated Point')

        # Draw the line from origin to the initial point
        pygame.draw.line(self.screen, (0, 255, 0),
                         (self.width // 2, self.height // 2),
                         (initial_point[0] * self.scaling_factor + self.width // 2,
                          self.height // 2 - initial_point[1] * self.scaling_factor), 2)

        # Draw the line from origin to the rotated point
        pygame.draw.line(self.screen, (255, 0, 0),
                         (self.width // 2, self.height // 2),
                         (rotated_point[0] * self.scaling_factor + self.width // 2,
                          self.height // 2 - rotated_point[1] * self.scaling_factor), 2)

        # Draw the coordinate points and their labels
        self.draw_coordinate_points()

        # Display the rotation angle
        angle_label = self.font.render(f'Rotation Angle: {angle} degrees', True, (255, 255, 255))
        self.screen.blit(angle_label, (self.width // 2 - 150, self.height // 2 - 30))
    ...
    def rotate_point(self, point, angle):
        ...
        x_rotated = point[0] * math.cos(angle_rad) - point[1] * math.sin(angle_rad)
        y_rotated = point[0] * math.sin(angle_rad) + point[1] * math.cos(angle_rad)
    ...
    def run(self):
        ...
        rotated_point = self.rotate_point(initial_point, angle)

        self.draw_axes(initial_point, rotated_point, angle)
